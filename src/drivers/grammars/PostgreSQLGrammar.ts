 ...
 */
export class PostgreSQLGrammar extends QueryGrammar {
  /**
  ...
  compileCreateTable(definition: TableDefinition): string {
    ...
    if (definition.inherits?.length) {
      const parents = definition.inherits.map((p: string) => this.wrapTable(p)).join(', ')
  ...
  compileAlterTable(definition: TableDefinition): string {
    ...
    const adds = definition.columns.map(col => {
      ...
      if (col.default != null) part += ` DEFAULT ${this.formatDefault(col.default)}`
  ...
  private compileColumnDefinition(column: ColumnDefinition): string {
    ...
    if (column.default != null) sql += ` DEFAULT ${this.formatDefault(column.default)}`
