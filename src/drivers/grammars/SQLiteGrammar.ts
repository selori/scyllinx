 ...
 */
export class SQLiteGrammar extends QueryGrammar {
  /**
  ...
  compileSelect(query: any): string {
    ...
    // FROM clause
    if (query.from) components.push(`FROM ${this.wrapTable(query.from)}`)

    // JOIN clauses
    if (query.joins?.length) components.push(this.compileJoins(query.joins))

    // WHERE clause
    if (query.wheres?.length) components.push(`WHERE ${this.compileWheres(query.wheres)}`)

    // GROUP BY clause
    if (query.groups?.length) {
      components.push(
        `GROUP BY ${query.groups.map((c: string) => this.wrapColumn(c)).join(', ')}`
      )
    }

    // HAVING clause
    if (query.havings?.length) components.push(`HAVING ${this.compileWheres(query.havings)}`)

    // ORDER BY clause
    if (query.orders?.length) {
      const orderStr = query.orders
        .map((o: any) => `${this.wrapColumn(o.column)} ${o.direction.toUpperCase()}`)
        .join(', ')
      components.push(`ORDER BY ${orderStr}`)
    }

    // LIMIT & OFFSET
    if (query.limit != null) components.push(`LIMIT ${query.limit}`)
    if (query.offset != null) components.push(`OFFSET ${query.offset}`)
  ...
  compileInsert(query: any): string {
    ...
    if (query.onConflict) sql += ` ON CONFLICT ${query.onConflict}`
    return sql
  }
   ...
   */
  private compileWheres(wheres: any[]): string {
   ...
   */
  private compileJoins(joins: any[]): string {
    return joins.map(j => {
   ...
   */
  private compileCtes(ctes: any[]): string {
   ...
   */
  private formatDefault(def: any): string {
   ...
   */
  private compileColumnDefinition(column: ColumnDefinition): string {
    ...
    if (column.default !== undefined) sql += ` DEFAULT ${this.formatDefault(column.default)}`
    return sql
  }
  ...
  compileAlterTable(definition: TableDefinition): string {
    ...
    const adds = definition.columns.map(col => {
      ...
      if (col.default !== undefined) part += ` DEFAULT ${this.formatDefault(col.default)}`
