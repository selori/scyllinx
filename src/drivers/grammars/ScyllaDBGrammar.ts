...
import type { WhereClause, QueryComponent, OrderClause, ColumnDefinition, TableDefinition } from "@/types"
 ...
 * 
 */
export class ScyllaDBGrammar extends QueryGrammar {
  /**
  ...
  compileSelect(query: QueryComponent): string {
    ...
    if (query.orders && query.orders.length > 0) {
      const orderBy = query.orders
        .map((order: OrderClause) => `${this.wrapColumn(order.column)} ${order.direction.toUpperCase()}`)
        .join(", ")
  ...
  compileAlterTable({ name, columns }: TableDefinition): string {
    ...
    const adds = columns.map(col =>
      `ADD ${this.wrapColumn(col.name)} ${this.getColumnType(col)}`
    )
  ...
  rename(from: string, to: string): Promise<void> {
    return Promise.reject(
      new Error(
        `Table rename from "${from}" to "${to}" is not supported by ScyllaDB.`
      )
    )
  }
}
