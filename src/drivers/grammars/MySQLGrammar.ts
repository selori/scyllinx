import { ColumnDefinition, TableDefinition } from "@/types/index"
 ...
 */
export class MySQLGrammar extends QueryGrammar {
  /**
  ...
  compileCreateTable(definition: TableDefinition): string {
    ...
    const cols = definition.columns.map((col: ColumnDefinition) => {
      const parts: string[] = []
      ...
      if (col.default != null) parts.push(`DEFAULT ${this.formatDefault(col.default)}`)
      return parts.join(' ')
    })
    const primaryCols = definition.columns.filter(c => c.primary).map(c => this.wrapColumn(c.name))
    if (primaryCols.length) cols.push(`PRIMARY KEY (${primaryCols.join(', ')})`)
  ...
  compileAlterTable(definition: TableDefinition): string {
    if (!definition.columns?.length) throw new Error('No columns provided for ALTER TABLE.')
    const table = this.wrapTable(definition.name)
    const adds = definition.columns.map((col: ColumnDefinition) => {
