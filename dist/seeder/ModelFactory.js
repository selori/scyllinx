...
const ModelRegistry_1 = require("@/model/ModelRegistry");
/**
...
class ModelFactory {
    ...
    async create(overrides = {}) {
        ...
        for (let i = 0; i < this.count; i++) {
            ...
            const ModelClass = ModelRegistry_1.ModelRegistry.getInstance().get(this.modelName);
    ...
    generateAttributes(overrides) {
        ...
        for (const state of this.currentStates) {
            ...
            const stateAttrs = typeof def === 'function' ? def(faker_1.faker) : def;
            attrs = { ...attrs, ...stateAttrs };
        }
        attrs = { ...attrs, ...overrides };
        const result = {};
        for (const [k, v] of Object.entries(attrs)) {
            result[k] = typeof v === 'function' ? v(faker_1.faker) : v;
        }
        return result;
    }
