...
export class MongoDBGrammar extends QueryGrammar {
     ...
     */
    compileAggregateExpressions(columns) {
        const result = {};
    ...
    compileCreateTable(definition) {
        ...
        for (const col of definition.columns) {
            ...
            if (col.minLength)
                prop.minLength = col.minLength;
            if (col.minimum)
                prop.minimum = col.minimum;
            if (col.maximum)
                prop.maximum = col.maximum;
            if (col.allowed)
                prop.enum = col.allowed;
            if (col.nullable === false)
                prop.nullable = false;
            if (col.default)
                prop.default = col.default;
            if (col.pattern)
                prop.default = col.pattern;
            if (col.format)
                prop.default = col.format;
            if (col.comment)
